...
-   在[Gitee](https://gitee.com/ascend/community)上Fork存储库。
...
1.  Fork community存储库。
    ...
    在向community代码仓提交代码之前，请确保该项目已经Fork到您自己的存储库。这意味着community存储库和您自己的存储库之间将存在并行开发，因此请注意避免存储库之间的不一致。
...
2.  克隆远程仓库。
    ...
    ```
    # For Gitee
    git clone https://gitee.com/ascend/community.git
    git remote add upstream https://gitee.com/ascend/community.git
    ```
...
3.  本地开发代码。
    ...
    以master分支为例，community可能会根据需要创建版本分支和下游开发分支，请先修复上游的bug。然后就可以随意更改代码了。
...
5.  向 community存储库拉取请求。

    在最后一步中，您需要在新分支和“community master“分支之间拉取比较请求。完成拉取请求后，“Jenkins CI“将自动设置为构建测试。您的pull request应该尽快合并到上游 master 分支，以降低合并的风险。
...
-   在[Gitee](https://gitee.com/ascend/community/issues)上提出您的想法作为_问题_。
